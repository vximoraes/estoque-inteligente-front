...
interface ModalEditarFornecedorProps {
  isOpen: boolean;
  onClose: () => void;
  fornecedorId: string;
  onSuccess?: () => void;
...
export default function ModalEditarFornecedor({
  ...
  const { setApiErrors } = useFormApiErrors<FornecedorUpdateFormData>(setError);
  ...
  const handleClose = () => {
    if (!updateFornecedorMutation.isPending) {
      onClose();
    }
  };
  ...
  const modalContent = (
    <ModalShell
      isOpen={isOpen}
      onClose={handleClose}
      data-test="modal-editar-fornecedor"
      zIndex={99999}
    ...
    >
      ...
      <div className="relative p-6 pb-0">
        <button
          data-test="modal-editar-fornecedor-close"
          onClick={handleClose}
      ...
      {isLoadingFornecedor ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 pb-6 space-y-5">
          ...
          {updateFornecedorMutation.error && (
            <div className="p-3 bg-destructive/10 border border-destructive/30 rounded-md text-sm text-destructive">
              <div className="font-medium mb-1">
                Não foi possível atualizar o fornecedor
              </div>
              <div className="text-destructive/80">
                {(updateFornecedorMutation.error as any)?.response?.data
                  ?.message ||
                  (updateFornecedorMutation.error as any)?.message ||
                  'Erro desconhecido'}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2">
            <Button
              data-test="modal-editar-fornecedor-cancelar"
              type="button"
              variant="outline"
              onClick={handleClose}
              disabled={isSubmitting || updateFornecedorMutation.isPending}
            ...
            <Button
              data-test="modal-editar-fornecedor-confirmar"
              type="submit"
              disabled={isSubmitting || updateFornecedorMutation.isPending}
