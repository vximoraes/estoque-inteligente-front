...
interface ModalCadastrarLocalizacaoProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export default function ModalCadastrarLocalizacao({
  isOpen,
  onClose,
  onSuccess,
}: ModalCadastrarLocalizacaoProps) {
  ...
  const { setApiErrors } = useFormApiErrors<LocalizacaoFormData>(setError);
  ...
  const cadastrarMutation = useMutation({
    mutationFn: async (data: LocalizacaoFormData) => {
      return await post('/localizacoes', data);
    },
    onSuccess: () => {
      queryClient.resetQueries({ queryKey: ['localizacoes'] });
      queryClient.resetQueries({ queryKey: ['localizacoes-infinite'] });
    ...
    onError: (error: any) => {
      ...
      toast.error(
        `Erro ao criar localização: ${error?.response?.data?.message || error.message}`,
        {
          position: 'bottom-right',
          autoClose: 5000,
        },
  ...
  const modalContent = (
    ...
    >
      ...
      <div className="relative p-6 pb-0">
        <button
          data-test="modal-cadastrar-localizacao-close"
          onClick={handleClose}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="px-6 pb-6 space-y-6">
        ...
        <div className="flex gap-3 pt-2">
          <Button
            data-test="modal-cadastrar-localizacao-cancelar"
            type="button"
            variant="outline"
            onClick={handleClose}
            disabled={isSubmitting || cadastrarMutation.isPending}
          ...
          <Button
            data-test="modal-cadastrar-localizacao-confirmar"
            type="submit"
            disabled={isSubmitting || cadastrarMutation.isPending}
