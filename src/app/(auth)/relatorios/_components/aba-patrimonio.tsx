...
import { generatePatrimonioPDF } from '@/utils/pdfGenerator';
import { generatePatrimonioCSV } from '@/utils/csvGenerator';
...
export default function AbaPatrimonio() {
  ...
  const handleSelectAll = () => {
    if (selectedItems.size === patrimoniosFiltrados.length) {
      setSelectedItems(new Set());
    } else {
      setSelectedItems(new Set(patrimoniosFiltrados.map((p) => p._id)));
    }
  };
  ...
  const isAllSelected =
    ...
    selectedItems.size === patrimoniosFiltrados.length;
  const isSomeSelected =
    selectedItems.size > 0 && selectedItems.size < patrimoniosFiltrados.length;

  const handleExport = async (fileName: string, format: string) => {
    try {
      const selecionados = patrimoniosFiltrados.filter((p) =>
        selectedItems.has(p._id),
      );
      ...
      if (format === 'PDF') {
        ...
        toast.success(
          `PDF gerado com sucesso! ${selecionados.length} unidade(s) exportada(s).`,
          { position: 'bottom-right', autoClose: 3000, transition: Slide },
        );
      } else {
        generatePatrimonioCSV({
          patrimonios: selecionados,
          fileName: fileName.trim(),
          includeStats: true,
        });
        toast.success(
          `CSV gerado com sucesso! ${selecionados.length} unidade(s) exportada(s).`,
          { position: 'bottom-right', autoClose: 3000, transition: Slide },
        );
      }
    ...
    >
      ...
      >
        ...
        <Button
          ...
          title={
            selectedItems.size === 0
              ? 'Selecione unidades para exportar'
              : `Exportar ${selectedItems.size} unidade(s)`
          }
        >
        ...
        {isLoading ? (
          ...
          <div className="border border-border rounded-md bg-card flex-1 overflow-hidden flex flex-col">
            <div className="overflow-x-auto overflow-y-auto flex-1 relative">
              <table className="w-full min-w-[950px] caption-bottom text-xs sm:text-sm">
                ...
                <TableBody>
                  {patrimoniosFiltrados.map((patrimonio) => (
                    <TableRow
                      ...
                      style={{ height: '60px' }}
                      onClick={() => handleSelectItem(patrimonio._id)}
                    >
                      <TableCell
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={selectedItems.has(patrimonio._id)}
                          onChange={() => handleSelectItem(patrimonio._id)}
                          className="w-4 h-4 accent-[var(--ei-accent)] cursor-pointer"
                      ...
                      >
                        <span
                          ...
                          title={patrimonio.modelo || patrimonio.categoria.nome}
                        >
