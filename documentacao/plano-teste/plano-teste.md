...
**Estoque Inteligente - Sistema de Gestão de Estoque Inteligente**

## 1 - Introdução

O Estoque Inteligente é um sistema web de gestão de itens. O sistema foi criado para gerenciar o ciclo completo de itens, incluindo cadastro, controle de estoque, movimentações (entradas e saídas), gerenciamento de fornecedores, orçamentos e geração de relatórios. Conta com funcionalidades de autenticação, gerenciamento de usuários, controle de permissões e recursos de exportação de dados. 

## 2 - Arquitetura

O sistema utiliza Next.js 15 com App Router como framework principal para o frontend, que possui uma arquitetura orientada a itens com React 19. A aplicação implementa Server-Side Rendering (SSR) e Client-Side Rendering (CSR) conforme necessário.
...
Para o armazenamento, consulta e alteração de dados da aplicação, o sistema consome uma API REST que disponibiliza endpoints para todas as entidades do sistema (itens, fornecedores, orçamentos, usuários, movimentações, etc.). A comunicação é feita através de requisições HTTP com autenticação via Bearer Token, retornando dados em formato JSON.
...
RF002 – O sistema deve permitir que usuários cadastrados acessem suas contas existentes para gerenciar seus itens, utilizando autenticação segura via JWT. | NF002 – O sistema deve implementar proteção de rotas autenticadas
RF003 – O sistema deve permitir gerenciar itens com campos essenciais, validando categoria/localização e nome único; ajuste de quantidade só por movimentação. | NF003 – O sistema deve ser acessível via navegadores modernos
...
RF005 – O sistema deve detalhar itens e atualizar estoque em tempo real.
...
RF007 – O sistema deve permitir a criação de orçamentos, informando nome e itens com seus devidos campos.
...
-----------|--------|--------|--------
Listagem de Itens | ● Ao entrar na tela, o usuário deve visualizar todos os itens cadastrados em formato de cards. <br> ● Cada card deve exibir imagem (ou ícone padrão), nome, categoria, quantidade, estoque mínimo e status.| ● Todos os itens do usuário são listados <br> ● Cards com informações corretas. | ● Itens carregados corretamente com suas devidas informações
Estatísticas | ● No topo da tela devem ser exibidos 4 cards com estatísticas. <br> ● Cards: Total de itens, Em estoque, Baixo estoque, Indisponível. <br> ● Cada card deve ter ícone e cor específica. <br> ● Contadores devem ser atualizados automaticamente após qualquer operação. | ● Cards sendo carregados corretamente <br> ● Contadores atualizando após operações de entrada e saída de itens | ● Estatísticas precisas <br> ● Atualização em tempo real
Pesquisa | ● Ao digitar no campo de pesquisa, deve filtrar itens por nome em tempo real. <br> ● Deve exibir mensagem "Nenhum item encontrado para sua pesquisa." quando não encontrar resultados. <br> ● Ao limpar o campo, deve voltar a exibir todos os itens. <br> ● Deve funcionar em conjunto com filtros aplicados. | ● Pesquisa em tempo real funcional <br> ● Busca por correspondência parcial <br> ● Mensagem quando não há resultados <br> ● Limpar campo restaura listagem <br> ● Compatível com filtros de categoria e status | ● Resultados corretos de busca
...
Remover Item | ● Botão de lixeira para remover item da tabela. <br> ● Ao remover, total do orçamento é recalculado. <br> ● Se remover todos, exibir mensagem "Nenhum item adicionado.". | ● Botão de remoção funcional <br> ● Total recalculado <br> ● Mensagem quando lista vazia | ● Remoção sem confirmação <br> ● Cálculos atualizados
Validações no Cadastro | ● Nome é obrigatório, exibir erro se vazio. <br> ● Pelo menos um item deve ser adicionado. <br> ● Todos itens devem ter fornecedor selecionado. <br> ● Mensagens de erro via toast. <br> ● Botão "Salvar" desabilitado durante processamento. | ● Erro se nome vazio <br> ● Erro se sem itens <br> ● Erro se item sem fornecedor <br> ● Toasts de erro claros <br> ● Estado de loading no botão | ● Validações claras <br> ● Não permite salvar inválido
Editar Orçamento | ● Ao clicar em "Editar" na listagem, redirecionar para tela de edição. <br> ● Campos pré-preenchidos com dados atuais. <br> ● Mesmas validações do cadastro. <br> ● Pode adicionar novos itens. <br> ● Pode remover itens existentes. <br> ● Pode alterar fornecedor, quantidade e valor de itens. <br> ● Ao salvar, exibir toast "Orçamento atualizado com sucesso!" e redirecionar. | ● Dados carregados corretamente <br> ● Validações mantidas <br> ● Adição de itens funciona <br> ● Remoção de itens funciona <br> ● Alteração de valores funciona <br> ● Toast de confirmação <br> ● Redirecionamento após salvar | ● Edição completa funcional <br> ● Dados persistidos corretamente
...
Estatísticas do Usuário | ● Exibir cards com estatísticas: Total de Itens, Total de Movimentações, Total de Orçamentos. <br> ● Valores devem ser numéricos. <br> ● Valores carregados dinamicamente. | ● Total de itens visível <br> ● Total de movimentações visível <br> ● Total de orçamentos visível <br> ● Valores numéricos corretos | ● Estatísticas precisas e atualizadas
...
Funcionalidades | Comportamento Esperado | Verificações | Critérios de Aceite
-----------|--------|--------|--------
Criação de Notificações | ● Ao cadastrar item, criar notificação correspondente. <br> ● Ao realizar entrada de item, gerar notificação. <br> ● Notificações aparecem na lista. | ● Notificação criada após cadastro <br> ● Notificação criada após entrada <br> ● Lista de notificações atualizada | ● Notificações geradas automaticamente
Mensagem de Status | ● Notificação deve exibir status correto do item. <br> ● Para "Em Estoque": mensagem com quantidade em estoque. <br> ● Para "Baixo Estoque": mensagem com quantidade baixa. <br> ● Para "Indisponível": mensagem com quantidade zero. <br> ● Mensagem deve incluir nome do item e quantidade. | ● Mensagem de "Em Estoque" correta <br> ● Mensagem de "Baixo Estoque" correta <br> ● Mensagem de "Indisponível" correta <br> ● Nome do item na mensagem <br> ● Quantidade na mensagem | ● Mensagens precisas e descritivas
...
Funcionalidades | Comportamento Esperado | Verificações | Critérios de Aceite
-----------|--------|--------|--------
Listagem de Relatório | ● Ao entrar na tela, exibir tabela com todos os itens e suas localizações. <br> ● Colunas da tabela: Checkbox, Código, Item, Quantidade, Status, Localização. <br> ● Cada linha representa um item em uma localização específica. <br> ● Todos os cabeçalhos devem estar visíveis e com nomes corretos. | ● Tabela carregada corretamente <br> ● Todos os cabeçalhos visíveis <br> ● Dados nas colunas corretas <br> ● Checkbox no header e em cada linha | ● Relatório completo e organizado
...
Filtros por Status | ● Botão "Filtros" para abrir modal de filtros. <br> ● Dropdown para selecionar status: Em Estoque, Baixo Estoque, Indisponível. <br> ● Ao aplicar filtro, exibir apenas itens do status selecionado. <br> ● Tag visual do filtro ativo com botão (X) para remover. <br> ● Modal fecha após aplicar filtros. | ● Modal de filtros abre e fecha <br> ● Dropdown de status funcional <br> ● Filtro aplicado corretamente <br> ● Apenas itens do status aparecem <br> ● Tag do filtro visível <br> ● Remoção do filtro funcional | ● Filtros precisos e fáceis de usar
...
O plano de testes abrange todas as funcionalidades descritas na tabela acima, incluindo itens, orçamentos, autenticação, cadastros auxiliares (categorias, localizações, fornecedores) e relatórios.
