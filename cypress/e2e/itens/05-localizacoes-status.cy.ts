    ...
    it('Deve abrir modal de localizações ao clicar no card', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itens.length > 0) {
          ...
          cy.intercept('GET', `${apiUrl}/estoques/item/${itemId}`).as(
            'getEstoques',
          );
          ...
          cy.contains(/localizações|onde está|estoque por localização/i).should(
            'be.visible',
          );
        }
      });
              ...
              cy.get('[role="dialog"]').within(() => {
                estoques.forEach((estoque: any) => {
                  if (estoque.quantidade > 0) {
                    ...
                    cy.contains(estoque.quantidade.toString()).should(
                      'be.visible',
                    );
                  }
                });
      ...
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itemComEstoque) {
          ...
          cy.get('[role="dialog"]').within(() => {
            ...
            cy.getByData('modal-localizacoes-total').should(
              'contain',
    ...
    it('Deve exibir mensagem apropriada quando não houver localizações', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itemSemEstoque) {
          ...
          cy.get('[role="dialog"]').within(() => {
            cy.contains(/nenhuma localização|sem estoque|não há/i, {
              timeout: 5000,
            }).should('be.visible');
          });
        }
      ...
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itemComEstoque) {
          ...
          cy.wait('@getEstoquesDetalhado').then((estoqueInterception) => {
            ...
            const estoques = Array.isArray(estoquesData)
              ? estoquesData
              : estoquesData?.docs || [];

            const totalCalculado = estoques.reduce(
              (sum: number, est: any) => sum + (est.quantidade || 0),
              0,
            );
    ...
    it('Deve calcular status "Baixo Estoque" quando 0 < quantidade < estoque_mínimo', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        const itemBaixoEstoque = itens.find(
          (c: any) => c.quantidade > 0 && c.quantidade < c.estoque_minimo,
        );
    ...
    it('Deve calcular status "Em Estoque" quando quantidade >= estoque_mínimo', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        const itemEmEstoque = itens.find(
          (c: any) => c.quantidade >= c.estoque_minimo && c.estoque_minimo > 0,
        );
    ...
    it('Deve atualizar status após entrada de estoque', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itemIndisponivel) {
          ...
          cy.intercept('POST', `${apiUrl}/movimentacoes`).as(
            'createMovimentacao',
          );
    ...
    it('Deve atualizar status ao alterar estoque mínimo', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itemComEstoque) {
          ...
          cy.intercept('PUT', `${apiUrl}/itens/${itemId}`).as(
            'updateComponente',
          );
          cy.intercept('PATCH', `${apiUrl}/itens/${itemId}`).as(
            'patchComponente',
          );
          cy.intercept('GET', `${apiUrl}/categorias*`).as('getCategorias');
          ...
          cy.wait(1000);
          cy.contains(itemComEstoque.nome)
    ...
    it('Deve sincronizar status com estatísticas', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (stats && itens.length > 0) {
          const indisponiveis = itens.filter(
            (c: any) => c.status === 'Indisponível',
          ).length;
          const baixoEstoque = itens.filter(
            (c: any) => c.status === 'Baixo Estoque',
          ).length;
          const emEstoque = itens.filter(
            (c: any) => c.status === 'Em Estoque',
          ).length;
    ...
    it('Deve manter consistência visual em toda aplicação', () => {
      cy.wait('@getComponentes').then((interception) => {
        ...
        if (itens.length > 0) {
          ...
          cy.get('[role="dialog"]').within(() => {
            cy.getByData('modal-localizacoes-titulo').should(
              'contain',
              primeiroItem.nome,
            );
          });
