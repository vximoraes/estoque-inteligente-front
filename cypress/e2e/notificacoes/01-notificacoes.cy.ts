describe('Notificações.', () => {
  ...
  const item = 'Componente Teste Notificações';
  const min = 5;
  ...
  it('Deve cadastrar um item e verificar se uma nova notificação correspondente é criada', () => {
    ...
    cy.wait(1500);
    cy.get('[data-test="stat-total-itens"]')
      .find('p')
      .invoke('text')
      .then((e) => {
        const total = parseInt(e.replace(/\D/g, ''));
        if (total == 0) {
          ...
          cy.get('[data-test="input-estoque-minimo"]')
            .clear()
            .type(min.toString());
          cy.get('[data-test="textarea-descricao-item"]').type(
            'Notificações teste...',
          );
        ...
        cy.get('[data-test="item-notificacao"]').should(
  ...
  it('Deve averiguar a mensagem sobre o status do último item.', () => {
    ...
    cy.get('[data-test="stat-total-itens"]')
      ...
      .then((e) => {
        ...
        cy.get('[data-test="item-card-0"]')
          .find('[data-test="quantity"]')
          .find('span')
          .first()
          .invoke('text')
          .then((qtdTextoInicial) => {
            const qtdInicial = parseInt(qtdTextoInicial.replaceAll(/\D/g, ''));
            ...
            if (statusInicial === 'Em Estoque') {
              ...
              cy.get('[data-test="item-card-0"]')
                ...
                .click();
              cy.get('[data-test="modal-saida-quantidade-input"]').type(
              ...
              cy.get('[data-test="item-card-0"]')
                .find('[data-test="entrada-icon"]')
                .click();
              cy.get('[data-test="modal-entrada-quantidade-input"]').type(
                movimentacao.toString(),
              );
            } else {
            ...
            cy.get(
              '[data-test="modal-entrada-localizacao-dropdown"], [data-test="modal-saida-localizacao-dropdown"]',
            ).click();
            cy.get(
              '[data-test="modal-entrada-localizacao-dropdown"], [data-test="modal-saida-localizacao-dropdown"]',
            )
              .parent()
              .find(
                'button:not([data-test="modal-entrada-localizacao-dropdown"]):not([data-test="modal-saida-localizacao-dropdown"])',
              )
              .first()
              .click();
            cy.get(
              '[data-test="modal-entrada-confirmar"], [data-test="modal-saida-confirmar"]',
            ).click();
            cy.wait(2000);
              ...
              .then((e) => {
                ...
                cy.get('[data-test="item-card-0"]')
                  ...
                  .then((e) => {
                    ...
                    cy.get('[data-test="botao-notificacoes"]').click();
                    cy.wait(2000);
                    cy.get('[data-test="mensagem-notificacao"]')
                      .first()
                      .invoke('text')
                      .then((notificacao) => {
                        if (e === 'Em Estoque') {
                          expect(notificacao).to.eq(
                            `${item} está em estoque (${quantidade} unidades)`,
                          );
                        } else if (e === 'Baixo Estoque') {
                          expect(notificacao).to.eq(
                            `${item} está com estoque baixo (${quantidade} unidades)`,
                          );
                        } else if (e === 'Indisponível') {
                          expect(notificacao).to.eq(
                            `${item} está indisponível (${quantidade} unidades)`,
                          );
                        }
                      });
                  });
      ...
      .then((e) => {
        ...
        if (total == 0) {
          ...
          cy.get('[data-test="modal-entrada-localizacao-dropdown"]')
            .parent()
            .find(
              'button:not([data-test="modal-entrada-localizacao-dropdown"])',
            )
            .first()
            .click();
          cy.get('[data-test="modal-entrada-confirmar"]').click();
      ...
      .then((e) => {
        ...
        cy.get('[data-test="quantity"]')
          ...
          .then((quantidadeTexto) => {
            const qtdAtual = parseInt(quantidadeTexto.replaceAll(/\D/g, ''));
            ...
            if (qtdAtual > 0) {
              ...
              cy.get('[data-test="modal-saida-localizacao-dropdown"]')
                .parent()
                .find(
                  'button:not([data-test="modal-saida-localizacao-dropdown"])',
                )
                .first()
                .click();
            ...
            cy.get('[data-test="quantity"]')
              ...
              .invoke('text')
              .then((qtdTexto2) => {
                const qtdAtual2 = parseInt(qtdTexto2.replaceAll(/\D/g, ''));
                cy.log(`Quantidade após entrada: ${qtdAtual2}`);

                // Calcular saída para deixar entre 1 e min-1 (Baixo Estoque)
                const saidaBaixoEstoque = qtdAtual2 - 3; // Vai ficar com 3 (1 <= 3 <= 4)
                cy.get('[data-test="item-card-0"]')
                  .find('[data-test="saida-icon"]')
                  .click();
                cy.get('[data-test="modal-saida-quantidade-input"]').type(
                  saidaBaixoEstoque.toString(),
                );
                cy.get(
                ...
                cy.get('[data-test="quantity"]')
                  ...
                  .invoke('text')
                  .then((qtdTexto3) => {
                    const qtdAtual3 = parseInt(qtdTexto3.replaceAll(/\D/g, ''));
...
function gerarNotificacao(min: number) {
  let valor = 0;
  cy.get('[data-test="contador-notificacoes"]')
    .invoke('text')
    .then((e) => {
  ...
  cy.get('[data-test="quantity"]')
    ...
    .invoke('text')
    .then((e) => {
      const qtd = parseInt(e.replaceAll(/\D/g, ''));
      cy.log(qtd.toString());
      if (qtd > min) {
        cy.get('[data-test="item-card-0"]')
          .find('[data-test="saida-icon"]')
