...
describe('Tela de relatórios de itens.', () => {
  ...
  describe('Validação da Tabela', () => {
    it('Deve verificar se os cabeçalhos da tabela estão corretos', () => {
      ...
      cy.get('[data-test="table-head-item"]').should(
        'contain.text',
        'COMPONENTE',
      );
      cy.get('[data-test="table-head-quantidade"]').should(
        'contain.text',
        'QUANTIDADE',
      );
      cy.get('[data-test="table-head-status"]').should(
        'contain.text',
        'STATUS',
      );
      cy.get('[data-test="table-head-localizacao"]').should(
        'contain.text',
        'LOCALIZAÇÃO',
      );
      ...
      cy.get('[data-test="item-status"]')
        ...
        .then((e) => {
          if (status.includes(e)) {
            ...
            cy.get('[data-test="filtro-status-dropdown"]')
              .parent()
              .find('div')
              .first()
              .contains('button', `${e}`)
            ...
            cy.get('[data-test="item-row"]').each((produto_status) => {
              const statusAtual = produto_status
                .find('[data-test="item-status"]')
                .text();
  ...
  describe('Busca de Componentes', () => {
    it('Deve pesquisar um item pelo nome.', () => {
      cy.get('[data-test="sidebar-btn-relatorios"]').click();
      cy.get('[data-test="sidebar-btn-relatorios-subitem-itens"]').click();
      cy.get('[data-test="item-row"]')
        .first()
        .find('[data-test="item-nome"]')
        .first()
        .invoke('text')
        .then((e) => {
