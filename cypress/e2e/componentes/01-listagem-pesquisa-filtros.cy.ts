describe('Componentes - Listagem, Pesquisa e Filtros', () => {
    ...
    cy.request({
      ...
      cy.request({
        ...
        if (compResponse.body.data.docs.length > 0) {
          primeiroComponente = compResponse.body.data.docs[0];
        }
      });
  ...
  beforeEach(() => {
    cy.intercept('GET', '**/componentes*').as('getComponentes');
    ...
    cy.get('[data-test="componentes-page"]', { timeout: 30000 }).should('exist');
    cy.wait('@getComponentes');
  });

  describe('Listagem de Componentes', () => {
    it('exibe grid de componentes após login', () => {
      cy.get('[data-test="componentes-grid"]').should('be.visible');
      cy.get('[data-test^="componente-card-"]').should('have.length.at.least', 1);
    });

    it('exibe informações do componente no card', () => {
      cy.get('[data-test="componente-card-0"]').should('be.visible');
      cy.get('[data-test="componente-card-0"]').within(() => {
    ...
    it('exibe estatísticas de componentes', () => {
      cy.viewport(1280, 720);
      cy.get('[data-test="stat-total-componentes"]').should('exist');
  ...
  describe('Pesquisa', () => {
    it('filtra componentes por nome', () => {
      expect(primeiroComponente).to.not.be.undefined;

      cy.intercept('GET', '**/componentes*').as('searchRequest');
      cy.get('[data-test="search-input"]').clear().type(primeiroComponente.nome);
      cy.wait('@searchRequest');
      cy.contains(primeiroComponente.nome).should('be.visible');
    });
    ...
    it('exibe mensagem quando não encontra resultados', () => {
      ...
      cy.contains('Nenhum componente encontrado para sua pesquisa.').should('be.visible');
    });
  ...
  describe('Filtros', () => {
    ...
    it('aplica filtro de status Em Estoque', () => {
      ...
      cy.intercept('GET', '**/componentes*').as('filterRequest');
    ...
    it('aplica filtro de categoria', () => {
      ...
      cy.intercept('GET', '**/componentes*').as('filterCategoria');
      cy.get('[data-test="aplicar-filtros-button"]').click();
  ...
  describe('Navegação', () => {
    it('redireciona para adicionar componente', () => {
      cy.get('[data-test="adicionar-button"]').click();
      cy.url().should('include', '/componentes/adicionar');
    });

    it('redireciona para editar componente ao clicar no botão editar', () => {
      cy.get('[data-test="componente-card-0"]').within(() => {
        cy.get('[data-test="edit-button"]').click();
      });
      cy.url().should('include', '/componentes/editar/');
    });
  });

  describe('Interações com Card', () => {
    it('abre modal de localizações ao clicar no card', () => {
      cy.intercept('GET', '**/estoques/componente/*').as('getEstoques');
      cy.get('[data-test="componente-card-0"]').click();
      cy.wait('@getEstoques');
