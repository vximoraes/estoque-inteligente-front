describe('Usuários - Detalhes e Reenvio de Convite', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('POST', `${apiUrl}/usuarios/*/reenviar-convite`).as('reenviarConvite');
    ...
    it('Deve copiar e-mail ao clicar no botão', () => {
      cy.get('body').then($body => {
        if ($body.find('[data-test="usuarios-table"]').length > 0) {
          ...
          cy.wait(500);
          cy.get('body').then($body => {
            if ($body.text().includes('copiado') || $body.text().includes('Copiado')) {
  ...
  describe('Navegação entre Usuários', () => {
    it('Deve poder visualizar detalhes de múltiplos usuários sequencialmente', () => {
      cy.get('body').then($body => {
        const totalUsuarios = $body.find('[data-test="usuarios-table"] tbody tr').length;
        ...
        if (totalUsuarios >= 2) {
          ...
          cy.log(`Apenas ${totalUsuarios} usuário(s) disponível(is) - não é possível testar navegação sequencial`);
        }
      });
    });
  });
