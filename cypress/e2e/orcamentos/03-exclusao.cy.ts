describe('Orçamentos - Exclusão', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('PATCH', `${apiUrl}/orcamentos/*/inativar`).as(
      'deleteOrcamento',
    );
        ...
        if ($body.find('[data-test="orcamentos-table"] tbody tr').length > 0) {
          ...
          cy.getByData('modal-excluir').within(() => {
            cy.getByData('modal-excluir-confirmar')
              ...
              .and(
                'match',
                /rgb\(2[012]\d, 3[0-9], 3[0-9]\)|rgb\(239, 68, 68\)|rgb\(248, 113, 113\)|rgb\(220, 38, 38\)|rgb\(185, 28, 28\)/,
              );
          });
        } else {
  ...
  describe('Processo de Exclusão', () => {
    it('Deve excluir orçamento ao confirmar (soft delete)', () => {
      ...
      cy.get('body').then(($body) => {
        if ($body.text().includes('Para Exclusão')) {
          ...
          cy.wait('@deleteOrcamento', { timeout: 30000 }).then(
            (interception) => {
    ...
    it('Deve atualizar listagem automaticamente após exclusão', () => {
      cy.get('body').then(($body) => {
        const totalAntes = $body.find(
          '[data-test="orcamentos-table"] tbody tr',
        ).length;
        ...
        if (totalAntes > 0) {
          ...
          cy.log(
            'Nenhum orçamento disponível para testar atualização da listagem',
          );
        }
      });
    });
    ...
    it('Deve exibir feedback apropriado após exclusão', () => {
      cy.get('body').then(($body) => {
        if ($body.find('[data-test="orcamentos-table"] tbody tr').length > 0) {
          ...
          cy.log(
            'Nenhum orçamento disponível para testar feedback de exclusão',
          );
        }
      });
    });
      ...
      cy.request({
        ...
        if (loginResponse.body?.data?.user?.accesstoken) {
          ...
          cy.request({
            ...
            cy.log(
              `Orçamento de teste ${orcamentoTesteId} removido na limpeza`,
            );
          });
        }
      });
