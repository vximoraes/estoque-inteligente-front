describe('Perfil — Informações e Estatísticas', () => {
  ...
  it('Renderiza área de notificações', () => {
    cy.get('[data-test="notificacoes-section"]').should('be.visible');

    cy.get('[data-test="notificacoes-section"]').within(() => {
      cy.get(
        '[data-test="notificacoes-list"], [data-test="no-notificacoes-message"]',
      ).then(($el) => {
        const temLista =
          $el.filter('[data-test="notificacoes-list"]').length > 0;
        const temVazio =
          $el.filter('[data-test="no-notificacoes-message"]').length > 0;
